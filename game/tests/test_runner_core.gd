...
class TestContext:
    ...
    func assert_equal(actual, expected, message: String = ""):
        ...
        var msg := message if message != "" else "Expected %s to equal %s" % [str(actual), str(expected)]
        _record_assertion(passed, msg)

    func assert_near(actual, expected, tolerance: float = DEFAULT_TOLERANCE, message: String = ""):
        var passed := _values_equal(actual, expected, tolerance)
        var msg := message if message != "" else "Expected %s to approximately equal %s (±%f)" % [str(actual), str(expected), tolerance]
        _record_assertion(passed, msg)
    ...
    func _values_equal(actual, expected, tolerance: float = DEFAULT_TOLERANCE) -> bool:
        if actual is float or expected is float:
            return is_equal_approx(float(actual), float(expected), tolerance)
        if (actual is Vector2 and expected is Vector2) or (actual is Vector2i and expected is Vector2i):
            return (Vector2(actual) - Vector2(expected)).length() <= tolerance
        if (actual is Vector3 and expected is Vector3) or (actual is Vector3i and expected is Vector3i):
            return (Vector3(actual) - Vector3(expected)).length() <= tolerance
...
class TestCaseResult:
    ...
    func _init(name: String, passed: bool, failure_messages: Array = []):
        self.name = name
        self.passed = passed
...
class TestResult:
    ...
    func _init(file_path: String, case_results: Array[TestCaseResult]):
        ...
        for path in test_paths:
            ...
            for case in result.case_results:
                if case.passed:
                    ...
                    var reason := case.failure_messages.is_empty() ? "Unknown failure" : "; ".join(case.failure_messages)
